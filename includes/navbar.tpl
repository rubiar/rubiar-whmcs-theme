{foreach $navbar as $item}
    <li menuItemName="{$item->getName()}" class="nav-item {if $item->hasChildren()}dropdown{/if}{if $item->getClass()} {$item->getClass()}{/if}" id="{$item->getId()}">
        <a class="nav-link" {if $item->hasChildren()}class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"{else}href="{$item->getUri()}"{/if}{if $item->getAttribute('target')} target="{$item->getAttribute('target')}"{/if}>
